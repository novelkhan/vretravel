...
import { MemberView } from '../../interfaces/User';


const Admin = () => {
    const [members, setMembers] = useState<MemberView[]>([]); // টাইপ ডিফাইন করুন
    const [memberToDelete, setMemberToDelete] = useState<MemberView | null>(null); // টাইপ ডিফাইন করুন
    const navigate = useNavigate();
  
    useEffect(() => {
      axios.get<MemberView[]>('/api/admin/get-members') // টাইপ ডিফাইন করুন
        .then(response => setMembers(response.data))
        .catch(error => console.log(error));
    }, []);
  
    const lockMember = (id: string) => {
